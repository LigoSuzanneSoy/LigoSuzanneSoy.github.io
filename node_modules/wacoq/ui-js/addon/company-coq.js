...
const special_tokens = {
    ...
    index_sub =      (mo) => {
        var res = [];
        for (var i = 1; i < mo[0].length; i++) {
            res.push({from: i, to: i+1, className: 'company-coq-sub', attributes: { 'data-company-coq-sub-content' : mo[0][i] }})
        }
        return res;
    },
    underscore_sub = (mo) => {
        var res = [{from: 0, to: 2, replacedWith: $('<span>')[0]}];
        for (var i = 2; i < mo[0].length; i++) {
            res.push({from: i, to: i+1, className: 'company-coq-sub', attributes: { 'data-company-coq-sub-content' : mo[0][i] }})
        }
        return res;
    },
    special_patterns = [
        ...
        {re: /H(\p{Lowercase_Letter}|\d)+$/u,      make: index_sub},
